...
def create_eventBridge_event(message):
    return {
        "Records": [
            {
...
def extract_context_from_sqs_or_sns_event_or_context(event, lambda_context):
    ...
    """
    print("extract_context_from_sqs_or_sns_event_or_context")
    try:
        first_record = event["Records"][0]
        first_record_eventBridge = False
        first_record_sns = False
        ...
        if "body" in first_record:
            ...
            try:
                ...
                if body.get("Type", "") == "Notification" and "TopicArn" in body:
                    logger.debug("Found SNS message inside SQS event1")
                    first_record_sns = get_first_record(create_sns_event(body))
            except Exception:
                first_record_sns = False
                first_record = event["Records"][0]
                pass
        ...
        if "body" in first_record:
            ...
            try:
                body = json.loads(body_str)
                if  body.get("detail", ""):
                    logger.debug("Found EventBridge message inside SQS event1")
                    first_record_eventBridge = get_first_record(create_sns_event(body))
            except Exception:
                first_record_eventBridge =False
                first_record = event["Records"][0]
                pass
        ...
        msg_attributes = {}
        if first_record_sns:
            msg_attributes = first_record_sns.get(
                "messageAttributes",
                first_record_sns.get("Sns", {}).get("MessageAttributes", {}),
            )
        if first_record_eventBridge:
            msg_attributes = first_record_eventBridge.get(
                "messageAttributes",
                first_record_eventBridge.get("Sns", {}).get("detail", {}).get("MessageAttributes", {}),
            )
        ...
        sampling_priority = dd_data.get(TraceHeader.SAMPLING_PRIORITY)
        print("extract_context_from_sqs_or_sns_event_or_context")
        print("trace_id", "parent_id", "sampling_priority")
        print(trace_id, parent_id, sampling_priority)
        return trace_id, parent_id, sampling_priority
...
def create_inferred_span_from_sqs_event(event, context):
    ...
    print("create_inferred_span_from_sqs_event")
    print(event_record)
    ...
    # logic to deal with SNS => SQS event
    sns_span = None
    eventBridge_span = None
    if "body" in event_record:
        body_str = event_record.get("body", {})
        try:
            body = json.loads(body_str)
            if body.get("Type", "") == "Notification" and "TopicArn" in body:
                logger.debug("Found SNS message inside SQS event2")
            ...
            if body.get("detail"):
                logger.debug("Found EventBridge message inside SQS event2")
                eventBridge_span = create_inferred_span_from_eventbridge_event(
                    body, context
                )
                eventBridge_span.finish(finish_time=start_time)
    ...
    if eventBridge_span:
        span.parent_id = eventBridge_span.span_id
    print("sqs_span")
    print(span)
    return span
...
def create_inferred_span_from_sns_event(event, context):
    event_record = get_first_record(event)
    print("create_inferred_span_from_sns_event")
    print(event_record)
...
def create_inferred_span_from_eventbridge_event(event, context):
    source = event["source"]
    print("create_inferred_span_from_eventbridge_event")
    print(event)
    print(source)
    ...
    # event_dt_format = "%Y-%m-%dT%H:%M:%S.%fZ"
    timestamp = event["time"]
    dt = datetime.strptime(timestamp, dt_format)
    # if event["detail"]["Timestamp"]:
    #     timestamp = event["detail"]["Timestamp"]
    #     dt = datetime.strptime(timestamp, event_dt_format)
    # else:
    #     timestamp = event["time"]
    #     dt = datetime.strptime(timestamp, dt_format)
